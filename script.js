...
let les_themes = ["aventure spatiale",
    ...
    "monstres de l'ombre",
    "magicien errant",
    "expérience scientifique interdite",
    "épidémie mystérieuse",
    "chasseurs de trésors",
    "héritage intergalactique",
    ...
    "accès à des dimensions parallèles par la musique",
    "armure magique alimentée par les émotions",
    "animal légendaire compagnon",
    "rêves prémonitoires",
    "mercenaires interstellaires",
    "objets magiques du zodiaque",
    "cristaux de mémoire",
    "cité engloutie ressurgissant",
    "chasse aux échos temporels",
    "métamorphose saisonnière",
    "festival des illusions",
    "oiseau messager mystique",
    "mirage galactique",
    "labyrinthe de l'esprit",
    "néant",
    "sortilèges oubliés dans les cavernes du temps",
    "cauchemars",
    "esprits errants",
    "faune extraterrestre pacifique",
    "constellations",
    "puzzle cosmique",
    "festival des feux follets",
    "mutation génétique bénéfique",
    "volcan magique",
    "rituel lunaire",
    "cataclysme climatique magique",
    "insectes dimensionnels",
    "pouvoirs liés aux émotions",
    "course de vaisseaux spatiaux dimensionnels",
    "sorcier maudit",
    "intelligence ancienne",
    "chasseurs de comètes",
    "contes de fées",
    "énergie stellaire",
    "voyage dans le miroir du temps",
    "société des rêveurs lucides",
    "planète des illusions",
    "expédition dans la nébuleuse des songes",
    "chasse au trésor interdimensionnelle",
    "sculptures de nuages",
    "magie quantique",
    "gardien de portail",
    "chasseurs de tempêtes dimensionnelles",
    "énergie magique des éclipses",
    "esprit emprisonné",
    "île des rêves brisés",
    "exploration des jardins stellaires",
    "révélation dans une bibliothèque cosmique",
    "pouvoir de manipuler les ombres",
    "voyage à travers les voiles du temps",
    "chasse aux étoiles filantes",
    "exploration de la cité des esprits",
    "cérémonie des étoiles dansantes","Nébuleuse",
    "Potion",
    "Abysses",
    "Chimère",
    "Mécanique",
...
let champ = document.getElementById("prompt")


function genere() {
    let prompt = ""
    let n_pokemon = randint(3)+1
    if (n_pokemon == 1 ){
        prompt +=  pokemon[randint(pokemon.length)]
    }
    else{
        prompt += "Fusioner "
    ...
    if (n_type > 0 ){
        prompt += " de type "
        for(i=0;i<n_type;i++){
            prompt += type_[randint(type_.length)] 
            if (i+1<n_type){
                prompt += ", "
        }}
    ...
    let theme = randint(2)
    if (theme>0){
        prompt += " dans le theme " + les_themes[randint(les_themes.length)]
    }
    champ.innerText = prompt
